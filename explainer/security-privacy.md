...
Web Agent Markup exposes **policy declarations** (visibility, mutability, retention rules) and **intent metadata** (semantic categories, importance levels) that content authors explicitly attach to DOM elements. This exposure is necessary to:
...
- The `wam-policy-input="none"` token explicitly allows hiding sensitive content from LLM context
...
Web Agent Markup provides explicit mechanisms for **protecting PII**:

1. **`wam-policy-input="none"`**: Completely excludes content from LLM context
2. **`wam-policy-input="structure"`**: Replaces text content with `[REDACTED]` placeholder while preserving structure
3. **`wam-policy-memory="none"`**: Ensures ephemeral processing only; no retention
...
**Provenance citations** (`wam-provenance-citation`) may contain author names if explicitly provided, but this is author-controlled attribution, not automatic PII exposure.

## 2.4 How do the features in your specification deal with sensitive information?

Sensitive information handling is a core design goal:

- **Medical/Health content**: Authors can mark with `wam-policy-output="readonly"` to prevent modification
- **Financial data**: Can be hidden entirely with `wam-policy-input="none"`
- **Legal content**: The `wam-intent-category="legal-disclaimer"` signals "do not paraphrase"
- **Authentication tokens**: Should be in elements marked `wam-policy-input="none"`
...
The `wam-policy-memory` attribute controls retention:
...
The **Provenance Ledger** is session-scoped by default and follows the most restrictive `wam-policy-memory` value of affected elements.

## 2.6 Do the features in your specification expose information about the underlying platform to origins?

No. Web Agent Markup does not expose:
...
No. Web Agent Markup does not interact with device sensors (camera, microphone, GPS, accelerometer, etc.).
...
1. **Provenance Ledger** (read-only): Origins can query what modifications the LLM has made within their document via `document.wamProvenanceLedger`. This is analogous to MutationObserver but filtered to LLM-initiated changes.

2. **Policy Resolution State**: `document.wamPolicyResolved` indicates when global policy is ready. This is similar to `document.readyState`.

3. **Effective Policy**: `element.wamPolicy` exposes the computed policy for an element, derived from explicit attributes and inheritance. This is similar to `getComputedStyle()` for CSS.
...
No. Web Agent Markup does not:
- Load external scripts
- Execute JavaScript
- Modify CSP or script execution policies

The `wam-policy-output="interaction"` token allows LLMs to modify event handlers on elements where explicitly permitted, but this operates within existing DOM security boundaries and requires explicit author permission.

## 2.10 Do features in this specification allow an origin to access other devices?

No. Web Agent Markup operates entirely within the document context and does not enable:
- Bluetooth access
- USB access
...
No. Web Agent Markup does not control:
- Browser chrome
- Address bar
- Tab management
...
**Agent IDs** (`wam-provenance-operation` may include `agentId`): If the LLM provides identification, this is logged. However:
- This is optional
...
1. **`wam-policy-memory`** MUST be treated as `none` regardless of attribute value
...
No. Web Agent Markup **cannot** weaken existing security:

- **CSP is unaffected**: Web Agent Markup does not modify Content Security Policy
- **Same-origin policy respected**: Cross-origin data remains inaccessible
- **Permissions unchanged**: Web Agent Markup doesn't grant camera, location, etc.

The `wam-policy-output="mutable"` allows LLM modifications but:
...
**License constraints cannot be overridden**: If content has `wam-policy-license="CC-BY-ND"`, explicit `wam-policy-output="mutable"` is intersected down to `readonly, annotation`.

## 2.17 How does your feature handle non-"fully active" documents?

Web Agent Markup follows standard document lifecycle:
...
The `document.wamPolicyResolved` state is preserved across bfcache navigation.
...
A: The `wam-provenance-operation` attribute and Provenance Ledger provide mandatory audit trails:
...
1. Mutations are validated against `wam-policy-output` before application
