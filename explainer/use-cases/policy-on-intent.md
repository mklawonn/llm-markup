...
The site designers cannot predict every possible CSS class or ID their legacy CMS might generate, but they can easily tag the semantic nature of the content using `llm-intent-category`.

### 1. Semantic Tagging (The "What")
The HTML is marked up with intent categories:

```html
<div llm-intent-category="historical-quote">
  "Four score and seven years ago..."
</div>
<div llm-intent-category="commentary">
...
2.  **Quotes:** When the agent attempts to rewrite the text inside `llm-intent-category="historical-quote"`, the browser **enforces the `readonly` policy**.
...
This demonstrates how `llm-intent-category` provides a stable hook for policy enforcement that is decoupled from visual presentation (CSS) and robust against styling changes.
